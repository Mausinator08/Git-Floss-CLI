...
const argv = yargs
	...
	.command('merge-release-master', 'Merges release into master and deletes the local release branch.')
	.command(
	...
	.command('delete-local-hotfix', 'Deletes the un-needed hotfix... (ONLY AFTER MERGING INTO MASTER AND DEVELOP!!!)', {
		version: {
...
try {
	if (argv._.includes('create-feature') === true) {
		...
		bumpReleaseVersion(argv.cwd ? argv.cwd : null, argv.nodejs ? (argv.nodejs === 'true' ? true : false) : false)
			.then((val) => {
				console.log(`[Date: ${dateFormater.toString(new Date(), 126, true)}]`);
				console.log(val);
				console.log('Done!');
